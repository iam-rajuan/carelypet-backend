...
export const listNotifications = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await notificationService.listForAudience("user", userId, query);
    res.json({
      success: true,
      data: {
...
export const markNotificationRead = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const notification = await notificationService.markRead("user", userId, req.params.id);
    res.json({
      success: true,
...
export const markAllNotificationsRead = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await notificationService.markAllRead("user", userId);
    res.json({
      success: true,
      data: { updated },
